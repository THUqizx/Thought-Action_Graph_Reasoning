...
def process_answers(
    ...
    """
    Process raw answers and format them for evaluation.
    
    This function handles answer formatting, including:
    - Splitting comma-separated answers
    ...
    for data in eval_data:
        ...
        for answer in answers:
            if ("," in answer[0][0]) and (" " in answer[0][0]):
                answer_temp = process_multiple_answers(answer[0][0])
                answer_processing.append(answer_temp)
            else:
...
if __name__ == "__main__":
    ...
    answer_path = config["data_paths"]["webqsp_raw_answer"]
    save_path = config["data_paths"]["webqsp_processed_answer"]
