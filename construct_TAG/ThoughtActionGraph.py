"""
Thought-Action Graph (TAG) data structure and operations.

This module provides the ThoughtActionGraph class for managing the dual-layer
knowledge graph structure used in Thought-Action Graph construction.
"""
...
class ThoughtActionGraph:
    ...
    The graph stores entities with their attributes and relationships between them.
    Each entity has a unique ID, name, and set of attributes.
    Relationships are stored as triples (head, relation, tail).
    """
    
    def __init__(self):
        """
        Initialize an empty Thought-Action Graph.
        """
        self.entities: Dict[str, Dict[str, Any]] = {}
        self.relations: Dict[str, Dict[str, List[str]]] = {}
        self.name_to_ids: Dict[str, List[str]] = {}
        self.next_id = 1
        self._node_cache: Dict[Tuple, List[Dict[str, Any]]] = {}
    ...
    def add_relation(self, head_id: str, relation: str, tail_id: str) -> None:
        ...
        if head_id not in self.entities or tail_id not in self.entities:
            raise ValueError("Head or tail entity does not exist in the graph")
    ...
    def _find_entity_with_attributes(self, name: str, attributes: Dict[str, Any]) -> Optional[str]:
        """
        Find an entity by name and exact attribute match.
        
        Args:
            name: Entity name.
            attributes: Entity attributes to match.
    ...
    def get_entity_by_id(self, entity_id: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Entity dictionary with 'name' and 'attributes', or None if not found.
        """
    ...
    def load_from_file(cls, file_path: str) -> 'ThoughtActionGraph':
        ...
        graph.name_to_ids = data["name_to_ids"]
    ...
    def count_entities_by_type(self, type_attr_name: str, target_type: Any) -> int:
        """
        Count entities with a specific type attribute value.
        
        Args:
            type_attr_name: Name of the attribute storing type information.
            target_type: Target type value to count.
            
        Returns:
            Number of entities with the specified type.
        """
        count = 0
    ...
    def count_entity_types(self, type_attr_name: str = "Type") -> Dict[str, int]:
        """
        Count entities grouped by their type attribute.
        ...
        for entity in self.entities.values():
            if type_attr_name not in entity["attributes"]:
                continue
            entity_type = entity["attributes"][type_attr_name]
    ...
    def count_relation_types(self) -> Dict[str, int]:
        ...
        Returns:
            Dictionary mapping relation names to counts.
        """
        relation_counts = {}
    ...
    def find_triples_with_tail_node(self, name: str, attrs: Dict[str, Any]) -> List[Tuple[Dict[str, Any], str, Dict[str, Any]]]:
        """
        Find all triples with a specific tail node.
    ...
    def find_triples_with_head_node(self, name: str, attrs: Dict[str, Any]) -> List[Tuple[Dict[str, Any], str, Dict[str, Any]]]:
        """
        Find all triples with a specific head node.
    ...
    def find_head_nodes_by_tail_and_relation(self, tail_name: str, tail_attrs: Dict[str, Any], relation: str) -> List[Dict[str, Any]]:
        ...
        Returns:
            List of head node dictionaries.
        """
        tail_id = self._find_entity_with_attributes(tail_name, tail_attrs)
        if tail_id is None:
            return []
    ...
    def get_related_relations(self, name: str, attrs: Dict[str, Any]) -> List[str]:
        """
        Get all relation types connected to a specific node.
        ...
        Returns:
            List of related relation names.
        """
    ...
    def _get_entity_id_by_data(self, entity_data: Dict[str, Any]) -> Optional[str]:
        """
        Get entity ID from entity data.
        
        Args:
            entity_data: Entity dictionary with 'name' and 'attributes'.
    ...
    def _traverse_next_end(self, 
        ...
        """
        Recursively traverse 'next' relationships until finding 'end' relationships.
        
        Args:
            current_node: Current node to process.
    ...
    def _traverse_to_target_end(self,
        ...
        """
        Recursively traverse 'next' relationships to find paths ending at target nodes.
        ...
        Args:
            ...
            all_valid_paths: List to collect all valid paths.
        """
        current_triples = self.find_triples_with_head_node(
            name=current_node["name"],
            attrs=current_node["attributes"]
        ...
        for end_triple in end_triples:
            complete_path = current_path.copy()
            complete_path.append(end_triple)
            end_tail_id = self._get_entity_id_by_data(end_triple[2])
            if end_tail_id in end_node_ids:
